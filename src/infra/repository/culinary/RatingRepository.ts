...
export class RatingRepository implements IRatingRepository {
  ...
  async getAllAsync(
    ...
    const [total] = await this.db.queryAsync(
      `select count(*) from recipe_ratings where ${where}`,
      [
        company,
  ...
  async deleteAsync(rating: Rating, date: Date): Promise<Rating> {
    await this.db.queryAsync(
      "update recipe_ratings set deleted_at=$3, updated_at=$3 where id = $1 and webeditor_companies_id = $2 and deleted_at is null",
